...
function ModelEnrollPage() {
    ...
    const fetchModel = async () => {
      ...
      try {
        ...
        if (modelData.modelCover && modelData.modelCover !== "default_cover.jpg") {
          try {
            // Fix: Use template literal for URL
            const imageResponse = await fetch(`https://eduvizbackend.onrender.com/model/${modelData.modelCover}`, {
              headers: { Authorization: `Bearer ${token}` }, // Fix: Proper string interpolation
              credentials: "include",
            });
            if (imageResponse.ok) {
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      const { data: keyData } = await axios.get("https://eduvizbackend.onrender.com/key");
      const { key } = keyData;

      const { data: orderData } = await axios.post("https://eduvizbackend.onrender.com/process", {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          try {
            ...
            await axios.post(
              "https://eduvizbackend.onrender.com/api/users/enroll",
              { modelId: model.id },
              {
                headers: { Authorization: `Bearer ${token}` }, // Fix: Proper string interpolation
                withCredentials: true,
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      await axios.post(
        "https://eduvizbackend.onrender.com/api/users/wishlist",
        { modelId: model.id },
        {
          headers: { Authorization: `Bearer ${token}` }, // Fix: Proper string interpolation
          withCredentials: true,
