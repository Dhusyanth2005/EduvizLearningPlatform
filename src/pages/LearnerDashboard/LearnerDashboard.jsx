...
function LearnerDashboard() {
                ...
                if (modelData.modelCover && modelData.modelCover !== "default_cover.jpg") {
                  try {
                    const imageResponse = await fetch(
                      `https://eduvizbackend.onrender.com/model/${modelData.modelCover}`,
                      {
                        headers: { Authorization: `Bearer ${token}` },
                        credentials: "include",
                      }
  ...
  useEffect(() => {
    const fetchAllModels = async () => {
      ...
      try {
        const response = await fetch("https://eduvizbackend.onrender.com/api/models/all", {
          headers: { Authorization: `Bearer ${token}` },
          credentials: "include",
        });
        ...
        const mappedModels = await Promise.all(
          modelData.map(async (model) => {
            ...
            if (model.modelCover && model.modelCover !== "default_cover.jpg") {
              try {
                const imageResponse = await fetch(
                  `https://eduvizbackend.onrender.com/model/${model.modelCover}`,
                  {
                    headers: { Authorization: `Bearer ${token}` },
                    credentials: "include",
                  }
    ...
    if (token) {
      try {
        const response = await axios.put(
          `https://eduvizbackend.onrender.com/api/users/wishlist/remove/${modelId}`,
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
