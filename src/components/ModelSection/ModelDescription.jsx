...
function ModelDescription({ selectedPart, isDarkMode, data }) {
  ...
  useEffect(() => {
    if (selectedPart) {
      ...
      const preloadData = async () => {
        try {
          await axios.post('https://eduvizbackend.onrender.com/preload', {
            data: {
  ...
  const sendMessage = async () => {
    ...
    if (userMsg === 'start') {
      try {
        const response = await axios.post('https://eduvizbackend.onrender.com/chat', { 
      ...
      try {
        const response = await axios.post('https://eduvizbackend.onrender.com/chat', { message });
        const botReply = response.data.reply;
        processResponse(botReply);
      } catch (error) {
        console.error('Chat error:', error);
